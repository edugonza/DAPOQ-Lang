...
join (
select distinct C.id as "id", max(E.timestamp) as "A_t", min(B.timestamp) as "B_t" from
"case" as C
...
join activity as ACTA on ACTA.id = AI.activity_id AND ACTA.name = "INFOSATZ_U_EFFPR"
join event as E on E.activity_instance_id = AI.id
join case_to_log as CTL on CTL.case_id = C.id
join log as LG on LG.id = CTL.log_id and LG.name = "log_p1018_2"
join (
select distinct C.id as "id", E.timestamp as "timestamp" from
"case" as C
join activity_instance_to_case as AITC on AITC.case_id = C.id
join activity_instance as AI on AI.id = AITC.activity_instance_id
join activity as ACTA on ACTA.id = AI.activity_id AND ACTA.name = "EINKBELEG_I_KEY"
join event as E on E.activity_instance_id = AI.id
join case_to_log as CTL on CTL.case_id = C.id
join log as LG on LG.id = CTL.log_id and LG.name = "log_p1018_2"
) as B on B.id = C.id
